...
class PlanningCurtainTest {
    ...
    fun `pulling further reaches the further door, and never skips one`() {
        ...
        assertEquals(PlanningDoor.Bill, armedDoor(1f))
    }
    ...
    fun `the dead band is wide enough to be a band and not a hair`() {
        ...
        // the three states needs a real share of the travel, or the pull is a coin flip.
        assertEquals(true, GigDetent >= 0.25f)
        assertEquals(true, BillDetent - GigDetent >= 0.25f)
        assertEquals(true, 1f - BillDetent >= 0.1f)
    }
    ...
    fun `overshoot past the end still means the last door`() {
        ...
        assertEquals(PlanningDoor.Bill, armedDoor(1.4f))
    }
}
