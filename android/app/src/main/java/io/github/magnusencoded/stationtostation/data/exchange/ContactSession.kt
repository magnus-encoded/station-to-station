...
fun runContactSession(
    ...
    /** Real bytes for a media id I might be asked to send, or null if I no longer have it. */
    mediaFile: (id: String) -> File?,
    /** Where a received item's bytes land, named for its id — the caller's to place. */
    receivedFile: (id: String) -> File,
    ...
    if (isServer) {
        sendRequested(socket, theirRequest, mediaFile)
        resolved += receiveRequested(socket, receivedFile)
    } else {
        resolved += receiveRequested(socket, receivedFile)
        sendRequested(socket, theirRequest, mediaFile)
    }
...
private fun sendRequested(socket: Socket, ids: List<String>, mediaFile: (String) -> File?) {
    for (id in ids) {
        val file = mediaFile(id) ?: continue
        FileInputStream(file).use { writeItem(socket, id, file.length(), it) }
    }
 ...
 */
private fun receiveRequested(socket: Socket, receivedFile: (String) -> File): Map<String, String> {
