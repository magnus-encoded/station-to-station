...
/// The **Gig** a **Pass** that names no night belongs to: the latest **Check-in** still running (#499).
///
...
/// later, and a night whose participation ended stops attracting **Passes** at all.
///
...
func gossipActiveGigId(cache: TimelineCache, stoppedAt: Int64 = 0, now: Int64) -> String? {
    ...
    return cache.gigs.values
        ...
        .compactMap { gig -> (id: String, at: Int64)? in
            (attendance[gig.setlistId ?? gig.id]?.checkedInAt).map { (id: gig.id, at: $0) }
        }
        .max { ($0.at, $0.id) < ($1.at, $1.id) }?.id
}
...
/// Anything read back per night has to union over the set rather than pick one, or adopting an id
/// silently splits a night's record in two — and unioning *by id* would then count the same
