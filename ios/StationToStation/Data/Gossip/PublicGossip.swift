...
/// Who a **Gig**'s **Room** can still say was there, and how many more it cannot name (#499).
///
...
/// and must die with the process; this answers "who was there", and a night does not stop having
...
struct PublicGossipState: Codable {
    ...
    mutating func rememberPass(peer: String, batch: [GossipEnvelope], activeGigId: String?, now: Int64) {
    ...
    func seenWith(gigIds: Set<String>, live: [String: String] = [:]) -> SeenWith {
        ...
        func order(_ a: (key: String, value: Int64), _ b: (key: String, value: Int64)) -> Bool {
            a.value != b.value ? a.value > b.value : a.key < b.key
        }
        let met = directAt.filter { name($0.key) != nil }.sorted(by: order)
        let onlyCheckedIn = checkedInAt.filter { directAt[$0.key] == nil && name($0.key) != nil }.sorted(by: order)
        return SeenWith(
...
/// in. The unnamed devices are counted rather than listed because there is nothing to list — a
/// **Blind relay** carries a key, never a person — and they are still said out loud, because
...
func seenWithLine(_ seen: SeenWith) -> String {
    ...
    let parts = [seen.named.joined(separator: ", ").nilIfBlank, others].compactMap { $0 }
