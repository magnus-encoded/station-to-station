...
struct StoredLog: Codable, Equatable {
    ...
    func absorbing(_ other: StoredLog) -> StoredLog {
        ...
        if lcs[0][0] == theirs.count { return self }
        var merged: [Entry] = []
        var a = 0, b = 0
        while a < mine.count || b < theirs.count {
            if a < mine.count, b < theirs.count, mine[a] == theirs[b] {
                merged.append(mine[a]); a += 1; b += 1
            } else if b == theirs.count || (a < mine.count && lcs[a + 1][b] >= lcs[a][b + 1]) {
                merged.append(mine[a]); a += 1
            } else {
                merged.append(theirs[b]); b += 1
            }
        }
        let ordered = merged.allSatisfy { $0.at != 0 }
            ? merged.enumerated().sorted { ($0.element.at, $0.offset) < ($1.element.at, $1.offset) }.map(\.element)
            : merged
        return StoredLog(songs: ordered.map(\.song), closed: closed,
                         remembered: ordered.map(\.line), enteredAt: ordered.map(\.at))
    }
