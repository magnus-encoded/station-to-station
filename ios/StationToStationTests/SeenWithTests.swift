...
///
/// Case for case with Android's `SeenWithTest`. Its subject is the *evidence*, not the radio,
/// which is why everything here is `PublicGossipState` and nothing here is a CoreBluetooth
/// callback. A completed **Pass** is the one thing the transport has to tell it about, through
/// `PublicGossipState.rememberPass`; a **Check-in** it already holds as a **Fact**.
final class SeenWithTests: XCTestCase {
    ...
    func testDirectPassesOrderTheNamesAndStrangersAreCountedNotNamed() throws {
        ...
        let ada = pair(), bo = pair(), adaGig = pair(), boGig = pair()
        XCTAssertTrue(state.receive(try checkIn(gig: adaGig, card: ada, scope: "ada-scope", at: 1000), from: key(adaGig), now: 1100))
        XCTAssertTrue(state.receive(try checkIn(gig: boGig, card: bo, scope: "bo-scope", at: 1200), from: key(boGig), now: 1300))
        state.recognizeContacts([key(ada), key(bo)], names: [key(ada): "Ada", key(bo): "Bo"])
        // Two Facts admitted and nothing met: only a completed Pass writes a device down, and an
        // advertisement — which reaches nothing in here at all — certainly does not.
        XCTAssertTrue(state.metDevices.isEmpty)

        state.rememberPass(peer: key(adaGig), batch: [], activeGigId: "gig", now: 5000)
        state.rememberPass(peer: key(boGig), batch: [], activeGigId: "gig", now: 6000)
        state.rememberPass(peer: "a-stranger-relay-key", batch: [], activeGigId: "gig", now: 7000)
        // Met again later: Ada moves ahead of Bo, and nothing is duplicated.
        state.rememberPass(peer: key(adaGig), batch: [], activeGigId: "gig", now: 8000)
    ...
    /// even when a witness **Carried** the last hop — attribution is the gate, not proximity — and
    /// they sort after everyone actually met.
    func testAWitnessCarriedCheckInNamesItsContactWithoutRaisingTheDeviceCount() throws {
        var state = PublicGossipState()
        let ada = pair(), cleo = pair(), adaGig = pair(), cleoGig = pair()
        ...
        state.rememberPass(peer: key(adaGig), batch: [], activeGigId: "gig", now: 3000)
    ...
    func testAPassLandsOnTheGigItsOwnClaimNamesOrElseOnTheActiveGig() throws {
        ...
        state.rememberPass(peer: key(theirs), batch: [claim], activeGigId: "tonight", now: 4000)
        ...
        state.rememberPass(peer: "a-relay-key", batch: [relayed], activeGigId: "tonight", now: 5000)
    ...
    func testABlockedContactIsCountedAnonymouslyWhenMetAndNotAtAllWhenOnlyRelayed() throws {
        var state = PublicGossipState()
        let blockedCard = pair(), blockedGig = pair(), awayCard = pair(), awayGig = pair()
        XCTAssertTrue(state.receive(try checkIn(gig: blockedGig, card: blockedCard, scope: "blocked-scope", at: 1000), from: key(blockedGig), now: 1100))
        // A relayed request from anyone but its author is refused by the one-hop rule.
        XCTAssertFalse(state.receive(try checkIn(gig: awayGig, card: awayCard, scope: "away-scope", at: 1200), from: "some-relay", now: 1300))
        ...
        state.rememberPass(peer: key(blockedGig), batch: [], activeGigId: "gig", now: 4000)
    ...
    func testTheRecordSurvivesRestartAndOutlivesTheNightsGossip() throws {
        var state = PublicGossipState()
        let ada = pair(), adaGig = pair()
        XCTAssertTrue(state.receive(try checkIn(gig: adaGig, card: ada, scope: "ada-scope", at: 1000), from: key(adaGig), now: 1100))
        state.recognizeContacts([key(ada)], names: [key(ada): "Ada"])
        state.rememberPass(peer: key(adaGig), batch: [], activeGigId: "gig", now: 2000)
        state.rememberPass(peer: "stranger", batch: [], activeGigId: "gig", now: 3000)
    ...
    func testAnAdoptedGigIdSeesOneRecordAndNotTwo() {
        var state = PublicGossipState()
        state.rememberPass(peer: "their-relay", batch: [], activeGigId: "local-id", now: 1000)
        state.rememberPass(peer: "their-relay", batch: [], activeGigId: "setlist-id", now: 2000)
    ...
    /// is named once *and* counted once among the others. This device cannot see that they are the
    /// same phone, and folding them would be it asserting something it does not know.
    func testAContactMetOnBothKeysIsNamedOnceAndStillCountedOnce() throws {
        ...
        state.recognizeContacts([key(ada)], names: [key(ada): "Ada"])
        state.rememberPass(peer: key(adaGig), batch: [], activeGigId: "gig", now: 2000)
        state.rememberPass(peer: "adas-nightly-relay-key", batch: [], activeGigId: "gig", now: 3000)
    ...
    // --- Helpers, in the shape `PublicGossipTests` already uses ---

    private func pair() -> P256.Signing.PrivateKey { P256.Signing.PrivateKey() }
    private func key(_ of: P256.Signing.PrivateKey) -> String { of.publicKey.derRepresentation.base64EncodedString() }
