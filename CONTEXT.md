...
**This file is the entry point and the authored source.** `UBIQUITOUS_LANGUAGE.md` is the
`domain-modeling` skill's model of the same vocabulary and is regenerated by it — do not
...
into the past. The one exception is a **Collection**, whose **Gigs** run in running order —
earliest first — because inside an evening the night's own order wins. The axis flips at the
night boundary, and a **Collection** is the one place both conventions are on screen at once.
...
| **Ticket** | A document shared into the app (a PDF, #411): it yields one or more **Admissions** and best-effort text (the text twice, from the PDF's own text layer and by OCR, the two readings cross-checked field by field, #526), and — short of a complete, unambiguous read — is always put in front of a person to confirm before it becomes anything. A complete read of a future night with no match mints a **Gig** the same way typing one in by hand does. Its **Admissions** outlive a failed read and stay with whatever **Gig** it ends up against; a second **Ticket** for that **Gig** adds its **Admissions** rather than replacing them. | boarding pass (unrelated), scan |
| **Admission** | One scannable barcode on a **Ticket** — the right of entry for one person. Kept exactly as printed, in the symbology it was printed in, and redrawn in that symbology — read back at import, so one the app cannot re-present is said at the prompt rather than found at the door; a **Ticket** for two people yields two, and the same barcode repeated on every page is still one. *Corroborated* when the **Ticket**'s own text also prints what the barcode says — evidence, never a condition for keeping it. Retired from the **Room** by the check-in. | QR (it may not be one), ticket (the document), barcode (the symbol, not the entitlement) |
...
**Down is later.** Within a night the order is the night's own, as it was played. Nothing in a
**Room** is ordered newest-first.
...
**Depth is later.** The corridor is the **Room**'s axis laid into the screen — dragging up walks
forward into the night, the same gesture-to-time mapping the **Room** has. Across a
**Collection**'s walk, depth is *running order*, not elapsed time: the hours between nights are
not walked.
...
| Term | Definition | Aliases to avoid |
| ---- | ---------- | ---------------- |
| **Flyover** | The landscape face of a **Gig**: the night as a corridor you travel down, media standing along it in the order it was captured. **Vertical is time** still holds — the corridor *is* the time axis, laid into the screen (within one night; across a **Collection**'s walk it is running order, see above). | 3D view, gallery, carousel, slideshow |
...
| **Presence row** | The line in a **Gig**'s **Room** that says this phone is standing at that night — the check-in mark, the bullet that says whether **Gossip** is running for it (amber for the **Active Gig**, dim for a night that could be), and the **Seen with** line under it. The one place the **Active Gig** is chosen. | status bar, header, toggle |
