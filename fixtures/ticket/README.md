...
5. **Corroborated** when the payload, read as strict UTF-8 with whitespace and `*` taken
   out, appears inside some line of some reading with the same taken out (`*K8TZC46G7*`
   corroborates `K8TZC46G7`). Every reading's raw lines count, before tidying. A payload
